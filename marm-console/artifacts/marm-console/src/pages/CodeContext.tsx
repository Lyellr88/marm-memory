...
import { Sparkles, FileCode2, Brain, FileText, AlertTriangle } from 'lucide-react';
    ...
    <div className="page-enter flex h-full flex-col overflow-hidden p-7 xl:p-8">
      ...
      <form onSubmit={submit} className="mb-6 shrink-0 space-y-3">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[16rem] flex-1">
            ...
            >
              <option value="">Resolve from the Console's working directory</option>
              {projects?.map((item) => (
      ...
      <div className="min-h-0 flex-1 overflow-auto">
          ...
          <div className="space-y-4">
            ...
            <Tabs defaultValue="symbols">
              <TabsList className="justify-start rounded-none border-0 bg-transparent p-0">
                <TabsTrigger value="symbols" className="rounded-none border-b-2 border-transparent px-5 py-3 data-[state=active]:border-primary data-[state=active]:bg-transparent">
                  <FileCode2 className="mr-2 h-4 w-4" /> Ranked symbols
                </TabsTrigger>
                <TabsTrigger value="memory" className="rounded-none border-b-2 border-transparent px-5 py-3 data-[state=active]:border-primary data-[state=active]:bg-transparent">
                  <Brain className="mr-2 h-4 w-4" /> What memory knows
                </TabsTrigger>
                <TabsTrigger value="agent" className="rounded-none border-b-2 border-transparent px-5 py-3 data-[state=active]:border-primary data-[state=active]:bg-transparent">
                  <FileText className="mr-2 h-4 w-4" /> Agent view
                </TabsTrigger>
              </TabsList>
        ...
        {!result && !errorMessage && !build.isPending && (
          <p className="text-sm text-muted-foreground">
            Describe a task above. Ranking is seeded from the task's own words, so a question about behaviour
