    ...
    <div className="page-enter h-full overflow-auto p-7 xl:p-8">
      ...
      <section className="mt-8 pb-8">
        ...
        {isLoading ? <div className="p-12 text-center text-muted-foreground">Loading projects…</div> : projects?.length === 0 ? (
          ...
          return <Card key={proj.name} className="project-card group metric-enter overflow-hidden border-border/70" style={{ animationDelay: `${index * 45}ms` }}><CardHeader className="flex flex-row items-start justify-between gap-4 pb-3"><div className="min-w-0"><div className="flex items-center gap-2"><CardTitle className="truncate font-mono text-base" title={proj.name}>{proj.display_name ?? proj.name}</CardTitle><Badge variant="outline" className={`shrink-0 border text-[10px] capitalize ${statusTone}`}>{status === 'indexing' && <RefreshCw className="mr-1 h-3 w-3 animate-spin" />}{status}</Badge></div><CardDescription className="mt-2 truncate font-mono text-xs" title={proj.root_path}>{proj.root_path}</CardDescription></div><div className="flex shrink-0 items-center gap-1"><Link href={`/explorer/${encodeURIComponent(proj.name)}`}><Button variant="outline" size="sm"><Compass className="mr-1.5 h-3.5 w-3.5" />Open in explorer</Button></Link><Button variant="ghost" size="icon" title="Prepare reindex" onClick={() => prepareReindex(proj)}><RefreshCw className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" title="Delete project graph" onClick={() => setDeleteTarget(proj)}><Trash2 className="h-4 w-4" /></Button></div></CardHeader><CardContent><div className="grid grid-cols-2 gap-3"><SmallStat label="Nodes" value={proj.nodes.toLocaleString()} caption="Files & symbols" /><SmallStat label="Edges" value={proj.edges.toLocaleString()} caption="Calls & imports" /></div></CardContent></Card>;
        })}</div>}
      </section>
