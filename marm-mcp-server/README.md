...
<h1 align="center">marm-memory v2.52.3 - Give your AI Agents a permanent memory in 60 seconds</h1>
...
| `marm_distill` | Turn raw conversation into memory proposals, each resolved against the store as `new`, `duplicate`, or `near`. Selects sentences verbatim by default; with `use_llm=true` and local generation enabled, writes self-contained facts with a local model instead, keeping the verbatim span each came from. Staged for review, never written unattended | `action="propose"\|"review"\|"apply"\|"discard"`, `text`, `session_name`, `proposal_id`, `use_llm`, `review_mode` (`manual`\|`guardrails`) |
...
| `marm_code_context` | Composed context for a task in one call: symbols ranked by personalised PageRank, their source read from disk, and what memory records about them | `task`, `project`, `cwd`, `budget`, `detail` (0-3; `0` uses `MARM_CODE_CONTEXT_DETAIL`, the server default), `include_graph` (default `false`), `answer`, `analyst_mode` (`read_only`\|`manual_review`\|`guardrails`) |
...
**By default it proposes and never writes.** `propose` stages into a review queue and only `apply` creates a memory; the one opt-in exception is `review_mode="guardrails"` with `MARM_ANALYST_AUTO_APPLY=1`, which applies a proposal only when every deterministic check passes. That is deliberately the same shape as `marm_compaction`, and for the same reason: a similarity score is not evidence enough to write memory unattended, and anything that does so on such a score fills a store with near-misses faster than it fills it with facts. A **discarded proposal is never proposed again**, enforced by a unique constraint rather than by convention — re-offering something a reviewer already rejected is how a review queue stops being read. It also makes re-running `propose` over the same text a no-op, which is what makes it safe to call at the end of every session.
...
`marm_code_context(answer=true)` is a bounded, read-only analyst. MARM retrieves; the local model answers only from what MARM retrieved; deterministic code judges the answer before it is returned.

- **One evidence packet.** The composition is frozen into a packet with citable handles, `S1`… for symbols and `M1`… for memories, and a content hash, `packet_id`. The model sees only the packet. The response carries the packet as `answer_packet`, and the stream sends it as a `packet` event.
- **Three states, scored by the weakest check.** `answer_verification` scores citation coverage, source-span support and graph/memory consistency, and the score is their minimum, never their average. The state is `verified`, `uncertain` or `rejected`, reported in `answer_status` as `ok`, `unverified` or `rejected`. Only evidence that contradicts the packet, such as a handle or identifier it does not contain, earns `rejected`; weak support is `uncertain`. An answer that says the context does not show something is `uncertain`, not `rejected`, and nothing from it is kept.
- **Budgets.** Context (`budget`, 12000 characters), output (`MARM_CODE_CONTEXT_ANSWER_TOKENS`, 900, one wider retry), time (`MARM_ANALYST_TIME_BUDGET`, 90 s) and follow-up retrievals (`MARM_ANALYST_FOLLOW_UPS`, 0, at most 2), every one of them MARM's own retrieval. A reader who closes the stream stops the generation. `answer_model_info` records the model, its endpoint source, the budget, the elapsed time and whether it stopped early.
- **Three modes** (`analyst_mode`). `read_only`, the default, returns the verified answer. `manual_review` also asks for at most three durable facts from it, verifies each against the same packet, and stages the verified ones as `marm_distill` proposals with `origin: "analyst"`, for a person to apply or discard. `guardrails` stages them the same way and then applies one only if every deterministic check passes.

**Nothing writes memory except `marm_distill apply`, or a guardrails decision the operator enabled.** `review_mode="guardrails"` on `marm_distill` makes the same decision for distilled proposals. The checks: `MARM_ANALYST_AUTO_APPLY` is exactly `1`; the resolver's verdict is `new`; one line of 12 to 300 characters; the span is verbatim in the source text (an analyst proposal must carry its cited source instead); an analyst proposal verified at a score of exactly 1.0; and nothing that looks like a secret. On any failure the proposal stays pending and the failing check is named. Every decision, applied or not, is recorded on the proposal before any write.
...
| `MARM_ANALYST_TIME_BUDGET` | `90` | Seconds an analyst answer may take, follow-ups included, 5 to 600. The stream stops generation at the deadline and says so |
| `MARM_ANALYST_FOLLOW_UPS` | `0` | Extra retrievals the analyst may run when its packet is not enough, at most `2`. Each one is MARM's own composition, never the model's |
| `MARM_ANALYST_AUTO_APPLY` | unset (off) | Must be exactly `1` for guardrails to apply anything. Anything else, including `true`, leaves every proposal pending for review |
