...
def test_dashboard_route_is_no_longer_mounted(monkeypatch, tmp_path):
    server = load_isolated_server(monkeypatch, tmp_path)
    client = local_client(server.app)
...
def test_dashboard_paths_are_not_public_auth_exceptions(monkeypatch, tmp_path):
    server = load_isolated_server(monkeypatch, tmp_path, api_key="test-key-123")
    client = remote_client(server.app)

    assert client.get("/dashboard").status_code == 401
    assert client.get("/dashboard/health").status_code == 401
    assert client.get("/dashboardevil").status_code == 401
