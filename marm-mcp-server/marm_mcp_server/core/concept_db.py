...
class ConceptDB:
    ...
    def get_or_create_entity(
        ...
        """Insert a new entity or append memory_id to an existing one's source list."""
        row = conn.execute(
        ...
        cursor = conn.execute(
            "INSERT INTO entities (name, type, session_name, project, source_memory_ids) "
            "VALUES (?, ?, ?, ?, ?)",
            (name, entity_type, session_name, project, json.dumps([memory_id])),
        )
