...
class MARMEvents:
        ...
        for i, callback in enumerate(callbacks):
            ...
            try:
                ...
                self._log_callback_error(
                    callback_id, "Callback timed out after 30 seconds", event_type
                )
    ...
    def _log_callback_error(self, callback_id: str, error_msg: str, event_type: str):
        ...
        if failure_count == 1:
            self.logger.warning(
                f"Event callback failed for '{event_type}': {error_msg}"
            )
        elif failure_count <= 5:
            self.logger.error(
                f"Event callback failed {failure_count} times for '{event_type}': {error_msg}"
            )
        else:
            self.logger.critical(
                f"Event callback consistently failing ({failure_count} times) for '{event_type}': {error_msg}"
            )
