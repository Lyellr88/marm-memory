...
Log only what matters -- decisions, breakthroughs, completions. Use `marm_smart_recall` before starting work on a known topic; it includes bounded concept/code context when a compatible graph exists. In an indexed repository, use `marm_code_context` for "how does X work" / "what would changing X affect" -- one call returns the ranked symbols, their source, and what memory knows. Use `marm_summary` at handoffs and end of sessions. Use `marm_notebook` for early ideas not ready to commit. Skip if the moment does not clearly fit.
