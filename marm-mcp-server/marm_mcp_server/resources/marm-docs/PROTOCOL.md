...
- Workflow: `marm_summary` (handoff/recap), `marm_delete` (explicit delete requests only), `marm_compaction` (agent-assisted memory cleanup), `marm_distill` (propose durable memories from raw conversation, resolved against what is already stored; nothing is written until a proposal is applied).
