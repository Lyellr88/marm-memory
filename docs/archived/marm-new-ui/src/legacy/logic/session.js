...
export function showSessionEntries(id, sessionName = null) {
  if (sessionName) {
    // Show specific session if it exists
    if (sessions[sessionName]) {
      const session = sessions[sessionName];
      let output = `📋 **Session: ${sessionName}**\n\n`;
      
      if (session.logs && session.logs.length > 0) {
        output += `**Log Entries (${session.logs.length}):**\n`;
      ...
      if (session.history && session.history.length > 0) {
        output += `\n**Chat History (${session.history.length} exchanges)**\n`;
      }
    ...
    let output = `📋 **All Sessions (${sessionIds.length})**\n\n`;
    sessionIds.forEach((sessionId, index) => {
      const session = sessions[sessionId];
      const logCount = session.logs ? session.logs.length : 0;
      const historyCount = session.history ? session.history.length : 0;
      const created = session.created ? new Date(session.created).toLocaleDateString() : 'Unknown';
      
      output += `${index + 1}. **${sessionId}**\n`;
      output += `   - Created: ${created}\n`;
      output += `   - Log entries: ${logCount}\n`;
      output += `   - Chat exchanges: ${historyCount}\n\n`;
    });
...
export function deleteLogEntry(id, target) {
  if (!target) {
    return "❌ Please specify what to delete (session name or entry).";
  }
  
  // Check if target is a session name
  if (sessions[target]) {
    delete sessions[target];
    persistSessions();
    return `✅ Session "${target}" deleted successfully.`;
  }
  
  // Check if target is a log entry in current session
  const currentSession = ensureSession(id);
  if (currentSession.logs && currentSession.logs.length > 0) {
    const originalLength = currentSession.logs.length;
    currentSession.logs = currentSession.logs.filter(log => !log.includes(target));
    
    if (currentSession.logs.length < originalLength) {
      persistSessions();
      return `✅ Log entry containing "${target}" deleted successfully.`;
    }
  }
  
  // Check all sessions for matching log entry
  for (const [sessionId, session] of Object.entries(sessions)) {
    if (session.logs && session.logs.length > 0) {
      const originalLength = session.logs.length;
      session.logs = session.logs.filter(log => !log.includes(target));
      
      if (session.logs.length < originalLength) {
        persistSessions();
        return `✅ Log entry containing "${target}" deleted from session "${sessionId}".`;
      }
    }
  }
  
  return `❌ No session or log entry found matching "${target}".`;
} 
