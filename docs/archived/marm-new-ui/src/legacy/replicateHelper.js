...
export async function generateContent(messages) {
  ...
  // Call backend server (original working setup)
 ...
 for (let attempt = 1; attempt <= maxAttempts; attempt++) {
  ...
  try {
    ...
    activeControllers.add(controller);
      ...
      if (!text) {
        console.error('[MARM DEBUG] Empty response from backend');
        return {
          text: () => '❌ Empty response from backend. Please try again later.'
        };
      }
      ...
      try {
        ...
        console.error('[MARM DEBUG] Failed to parse backend response as JSON:', text);
        return {
          text: () => '❌ Invalid response from backend. Please try again later.'
        };
      }
      ...
      if (data.response) {
        ...
        console.error('[MARM DEBUG] No response field in backend data:', data);
        return {
