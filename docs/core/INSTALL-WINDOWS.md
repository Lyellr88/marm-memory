...
**MARM v2.2.7* - Memory Accurate Response Mode with WebSocket Support
...
**If you prefer the `marm-mcp-server` CLI command**, add the Python Scripts folder to your PATH first:
...
Then restart your terminal and use either:
```powershell
marm-mcp-server        # CLI command (after PATH fix)
python -m marm_mcp_server  # always works without PATH fix
```
...
4. **Update Dependencies**:
   ...
   ```bash
   pip install marm-mcp-server==2.2.7 --upgrade
   ```
...
**MARM v2.2.7 Windows Guide** - *Universal memory intelligence for AI agents*
