...
### 5. Cost of Automatic Concept Indexing

What background indexing (v2.36.0) does to interactive latency while it is clearing a backlog. Measured on a real 768-memory corpus with [`bench_concept_worker.py`](scripts/benchmarking/performance/bench_concept_worker.py), worker draining the whole queue throughout, two runs.

| Path | Idle | While indexing | Median change |
| :--- | :--- | :--- | :--- |
| `store_memory` median | 10.2 - 11.2 ms | 10.8 - 11.1 ms | **-0.5% to +6%** |
| `recall_similar` median | 8.3 - 8.5 ms | 15.0 - 17.7 ms | **+80% to +109%** |
| `recall_similar` p95 | 11.2 - 12.3 ms | 28.0 - 34.1 ms | |

Writes are effectively unaffected. Recall roughly doubles in relative terms while staying under 20ms median, and this only applies while a backlog is draining, which for most installs is once, after the upgrade rebuild. Steady-state indexing of a few new memories leaves the worker idle almost all the time.

The cause is CPU contention from spaCy extraction, not lock waiting. Disabling entity-name embedding (the `memory._encoder_lock` path) leaves recall unchanged, so tuning the encoder would not help. Corpus shape matters: on short synthetic memories the same benchmark shows writes +65%, because many small extractions generate entity embeddings far faster than real content does. Trust the real-corpus row.

`CONCEPT_INDEX_BATCH_PAUSE_MS` (default 250) does not move p95. It exists for the tail, cutting worst-case recall during a drain from roughly 270ms to 80ms in exchange for about 18% longer drains.
...
- **Clearing a backlog costs some recall speed**: entity extraction is CPU-bound, so while the worker is working through a queue, measured recall goes from ~8ms to ~16ms median on a real 768-memory corpus. Writes are unaffected. It only applies while a backlog is draining, which for most people is once, after the upgrade rebuild. See [section 5 of the benchmarks](#5-cost-of-automatic-concept-indexing).
