...
<h1 align="center">MARM: Local-First Persistent Multi-Agent Memory Layer for MCP Clients v2.35.0</h1>
...
| `upgrade --check` | Compares the installed package against PyPI |
| `upgrade` | Previews a safe native upgrade; `--yes` performs it where the active installer can be replaced safely |
| `uninstall` | Previews removal; always preserves `~/.marm` (memory databases, graph indexes, keys, logs, configuration) |
