...
## MARM Systems – v2.5.3 Change Log
...
<details>
<summary>May 18th, 2026: MCP Registry Publishing & CI/CD Hardening (v2.5.3)</summary>

### Changed

- **CI/CD Pipeline rewrite**: Replaced the brittle build-from-source MCP registry job with the official `mcp-publisher` CLI workflow.
- **Authentication**: Migrated to GitHub OIDC for secure, passwordless authentication to the MCP Registry.
- **Schema Update**: Bumped `server.json` schema version to `2025-12-11` (latest).
- **Version Synchronized**: Package version bumped to v2.5.3 across `pyproject.toml` and `server.json`.

### Fixed

- **Publishing Reliability**: Eliminated complex Go build steps in CI, reducing job failure rate and improving deployment speed.
- **Registry Compatibility**: Unified the publishing process to match current Model Context Protocol standards.

### Chore

- **Repo Hygiene**: Untracked and gitignored agent config folders (`.claude`, `.codex`, `.gemini`, `.qwen`) and documentation workspace folders (`docs/archived`, `docs/current`, `docs/future`) to keep the repository clean.
- **Dashboard Cleanup**: Added `.pytest_tmp` to gitignore and untracked stale test artifacts in the `marm-dashboard` package.
